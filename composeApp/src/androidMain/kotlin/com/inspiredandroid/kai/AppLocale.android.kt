...
actual fun applyAppLocale(tag: String) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
        ...
        context.getSystemService(LocaleManager::class.java)?.applicationLocales =
            if (tag.isEmpty()) LocaleList.getEmptyLocaleList() else LocaleList.forLanguageTags(tag.toBcp47())
    } else {
        Locale.setDefault(if (tag.isEmpty()) Locale.getDefault() else Locale.forLanguageTags(tag.toBcp47()))
...
fun wrapContextLocale(base: Context, tag: String): Context {
    ...
    val locale = Locale.forLanguageTags(tag.toBcp47())
