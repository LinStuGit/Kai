...
object AgentClient {
    ...
    private fun executeTool(
        ...
        return when (name) {
            ...
            "overlay_show" -> {
                if (args.optString("action", "show") == "hide") {
                    AgentOverlayState.clearContent()
                    "已关闭悬浮窗内容"
                } else {
                    val url = args.optString("url").trim()
                    val image = args.optString("image").trim()
                    val html = args.optString("html")
                    val title = args.optString("title").ifBlank { "Kami 内容" }
                    when {
                        url.isNotEmpty() -> {
                            AgentOverlayState.showContent(OverlayContent("url", url, title))
                            "已在悬浮窗打开网页"
                        }

                        image.isNotEmpty() -> {
                            AgentOverlayState.showContent(OverlayContent("image", image, title))
                            "已在悬浮窗显示图片（若提示无法读取，改用 html+base64 内嵌）"
                        }

                        html.isNotEmpty() -> {
                            AgentOverlayState.showContent(OverlayContent("html", html, title))
                            "已在悬浮窗渲染 HTML"
                        }
            ...
            "vibrate" -> {
                val ms = args.optLong("duration_ms", 400L).coerceIn(50L, 5000L)
                Vibe.once(context, ms)
                "已震动 ${ms}ms"
            }

            "fullscreen_alert" -> try {
                context.startActivity(
                    Intent(context, FullscreenAlertActivity::class.java)
                        .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                        .putExtra("title", args.optString("title", "提醒"))
                        .putExtra("text", args.optString("text"))
                        .putExtra("vibrate", args.optBoolean("vibrate", true)),
                )
                "已弹出全屏提醒"
            } catch (t: Throwable) {
                "全屏提醒失败：${t.message}"
    ...
    private fun notifyUser(context: Context, args: JSONObject): String {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            ...
            nm.createNotificationChannel(
                NotificationChannel("kami_agent", "Kami Agent", NotificationManager.IMPORTANCE_HIGH).apply {
